...
class LineMarker {
    ...
    getDecorationTypeForFile(filePath) {
        if (!this.lineDecorationTypes.has(filePath)) {
            ...
            const style = config.get(constants_1.CONFIG_KEYS.LINE_MARK_STYLE) ||
                constants_1.DEFAULTS.LINE_MARK_STYLE;
            let decorationRenderOptions;
            if (style === constants_1.LineMarkStyle.INLINE) {
                ...
                const sidebarColor = config.get(constants_1.CONFIG_KEYS.SIDEBAR_COLOR) ||
                    constants_1.DEFAULTS.SIDEBAR_COLOR;
                decorationRenderOptions = {
    ...
    async toggleSelectedLines() {
        ...
        const allSelectedLinesMarked = selectedRanges.every((selectedRange) => existingDecorations.some((decoration) => decoration.range.start.line <= selectedRange.range.start.line &&
    ...
    async markSelectedLines() {
        ...
        for (const selection of selectedRanges) {
            for (let line = selection.range.start.line; line <= selection.range.end.line; line++) {
        ...
        for (let i = 1; i < combinedDecorations.length; i++) {
            ...
            if (currentDecoration.range.end.line + 1 >=
                nextDecoration.range.start.line) {
                // Merge them by extending the current decoration
                currentDecoration = {
                    range: new vscode.Range(currentDecoration.range.start.line, 0, Math.max(currentDecoration.range.end.line, nextDecoration.range.end.line), editor.document.lineAt(Math.max(currentDecoration.range.end.line, nextDecoration.range.end.line)).text.length),
                };
            }
            else {
        ...
        existingDecorations.forEach((decoration) => {
            ...
            for (let line = decoration.range.start.line; line <= decoration.range.end.line; line++) {
                if (!linesToUnmark.has(line)) {
                    ...
                    newDecorations.push({
                        range: new vscode.Range(currentStart, 0, currentEnd, editor.document.lineAt(currentEnd).text.length),
                    });
                    currentStart = line + 1;
                    currentEnd = line;
                }
    ...
    async clearAllLineMarksInProject() {
    ...
    async saveState() {
        ...
        await this.context.workspaceState.update(constants_1.STORAGE_KEYS.MARKED_LINES, markedLinesMap);
    }
    // Restore line marks state from workspace storage
    async restoreState() {
        const markedLinesMap = this.context.workspaceState.get(constants_1.STORAGE_KEYS.MARKED_LINES, {});
